...
libUtils.dateToYYYYMMDD = (date) => {
  const dtobj = this.dateToStrObj(date)
  return [dtobj.yyyy, dtobj.mm, dtobj.dd].join('')
}

libUtils.dateToYYYYMMDDhhiiss = (date) => {
  const dtobj = this.dateToStrObj(date)
  return [dtobj.yyyy, dtobj.mm, dtobj.dd, dtobj.hh, dtobj.ii, dtobj.ss].join('')
}

libUtils.dayToTh = (day) => {
  if (day === 1) return '1st'
  if (day === 2) return '2nd'
  if (day === 3) return '3rd'
  return day + 'th'
...
libUtils.monthToName = (mt) => {
  if (!MONTHS_LIST[mt]) return ''
  return MONTHS_LIST[mt].name
}

libUtils.dateToExtendedString = (date) => {
...
libUtils.weekIntervalToString = (dt1, dt2, type) => {
  let intStr = ''
