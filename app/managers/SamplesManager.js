class SamplesManager {
  ...
  _createIndexHolder (options) {
    const _self = this
    const __new_SamplesTree = function () {
      const STree = new SamplesTree(options.directoryToScan, {
        /* SampleTree options */
  ...
  setSamplesIndex (options) {
    ...
    if (options.force === true) {
      const smp_obj = dataFileHolder.set(this._LABEL_samples_index)
  ...
  searchSamplesByTags (tagString, random) {
    ...
    const _self = this
    const smp_obj_search = this._CACHE_stqall.get(tagString /* label */, function () {
      const ST = dataFileHolder.get(_self._LABEL_samples_index)
      if (!ST) return null

      const smp_obj2 = ST.filterByTags(tagString)
    ...
    const smp_obj_search_random = this._CACHE_stqrnd.get(tagString /* label */, function () {
      const smp_rnd_obj2 = smp_obj_search.getRandom(configMgr.get('RandomCount'), configMgr.get('MaxOccurrencesSameDirectory'))
  ...
  generateSamplesDir_setOptions (smp_obj, options) {
    ...
    if (!_.isString(options.path)) {
      ...
      options.path = Utils.File.pathJoin(ProjectsMgr.current, 'mpl', options.dirname)
    }
  ...
  generateSamplesDir (smp_obj, options) {
    ...
    const p_array = []
    const fname_array = []
    const smp_copied_obj = smp_obj.createFromThis()
    const _links_dir = Utils.File.pathJoin(options.path, '_links')
    ...
    smp_obj.forEach(function (item, index) {
      const f_name = _.noDuplicatedValues(fname_array, Utils.File.pathBasename(item.path), (v, cv, i, a) => {
      ...
      const link_file_name = f_name + '___' + Utils.replaceAll(item.path.substring(configMgr.cfg_path('SamplesDirectory').length), Utils.File.pathSeparator, '___')
  ...
  checkSamplesCoverage (options) {
    ...
    _data.tags.forEach((v, i, a) => {
      ...
      if (v.length > _data.output.max_length_tag_string) { _data.output.max_length_tag_string = v.length }
    })
    ...
    const coverage_output = this._checkSamplesCoverageOutput1(_data, options)
    coverage_output.unmatched_obj = _data.smpobj_unmatched.obj
    return coverage_output
  }

  _checkSamplesCoverageOutput1 (_data, options) {
    const coverage_output = { error: true }

    if (!_data.output.enabled) return coverage_output
    _data.smpobj_by_tag = Utils.sortObjectByKey(_data.smpobj_by_tag)

    const _max_len_size = ('' + _data.samples_count + '').length + 1
    const _big_separator = _.repeat('-', 120)

    const k_array = Object.keys(_data.smpobj_by_tag)
