...
class ExportManager {
  ...
  exportBookmarks (options) {
    ...
    return this.__export(function (archive, o /* parsed options */) {
      const archive_name = 'mpl_bookmarks'
      const archive_path = Utils.File.checkAndSetDuplicatedFileNameSync(Utils.File.pathJoin(o.destPath, archive_name + '_' + Utils.Date.dateToYYYYMMDD() + '.zip'))
  ...
  exportProject (options) {
    return this.__export(function (archive, o /* parsed options */) {
      const archive_path = Utils.File.checkAndSetDuplicatedFileNameSync(
  ...
  __export (configFn, options) {
    ...
    return new Promise(function (res, rej) {
      ...
      const archive = _self._ARCHIVER('zip', {
        zlib: { level: options.compressionLevel } // Sets the compression level.
      })

      const config = configFn(archive, options)

      // create a file to stream archive data to.
      const output = Utils.File._FS.createWriteStream(config.archive_path)
