class ProjectsTemplate {
  ...
  _remove (template_path) {
    const index = this.getIndex(template_path)
    if (index >= 0) {
  ...
  _newProjectName (project_name) {
    const project_np = configMgr.get('ProjectNamePattern')
    if (project_np.length <= 0) return project_name
    const new_project_name = Utils.replaceAll(project_np, '<name>', project_name)
  ...
  newProject (template_path, project_parent_path, project_name) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      const project_path_fn = _self._newRenameFn(project_parent_path, project_name)
