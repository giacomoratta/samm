class Samples {
  ...
  static processTagString (ts, splitFn) {
    ...
    tagOR.forEach(function (v1, i1, a1) {
      ...
      const tagAND = _.split(v1, '+')
      proc_obj.array.push([])
      const po_index = proc_obj.array.length - 1
  ...
  print (prefix, processFn) {
    const padding = ('' + this.size() + '').length + 2
  ...
  getRandom (count, max_occur, adapt_window) {
    ...
    const _sameDirectoryMaxOccurs = function (item, o_obj, max_o) {
      const f_path = item.dir
    ...
    let rf; let rn; const r_array = []
    const occur_obj = {}
    if (_.isNil(max_occur)) max_occur = -1

    // New object for random samples
    const smp_obj_random = this.createFromThis()

    while (max_attempts > 0) {
      let i = 0; let sec = size
      while (i < count && sec > 0) {
        sec--
