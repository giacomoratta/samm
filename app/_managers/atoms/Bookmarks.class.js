class Bookmarks {
  ...
  sort () {
    const bkeys = Object.keys(this._data)
    const _self = this
    bkeys.forEach((label) => {
  ...
  printIndexedList (printFn) {
    const keys = Object.keys(this._data)
    const printLabels = keys.length > 1
    let index = 0
    printFn('')
    for (let i = 0; i < keys.length; i++) {
  ...
  cloneStructure () {
    const newBookm = new this.constructor()
